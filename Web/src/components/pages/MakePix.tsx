...
export function MakePix(){
      ...
      async function sendData(){
         const response = await api.post('makepix',{
            email,
            amount,
            pixKey
         }) 
      ...
      return (
         <div className="h-screen flex items-center justify-center bg-cyan-900 uppercase text-white font-extrabold">
            <div className="shadow-xl shadow-zinc-300 bg-cyan-700 p-4 pt-10 border-2 rounded-lg border-zinc-300 h-[95%] w-2/5 flex flex-col gap-10 items-center justify-center">
               <h1 className="text-5xl font-bold ">Make pix</h1>
               ...
               <label htmlFor="pixKey" className="text-3xl block uppercase tracking-wide text-white font-bold mb-2">pix Key:</label>
               <input onChange={e => {setPixKey(e.target.value)}} type="text" id="pixKey" className="text-center font-bold text-3xl appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"/>
